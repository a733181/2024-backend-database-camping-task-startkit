         ...
         join "CREDIT_PACKAGE" as cp
              on
                  (u.name = '王小明' and cp.name = '14 堂組合包方案')
                      or
                  (u.name = '王小明' and cp.name = '21 堂組合包方案')
                      or
...
from "COACH" as c
         ...
         join "SKILL" as k
              on
                  (k.name = '重訓')
                      or
                  (k.name = '瑜伽' and u.email = 'muscle@hexschooltest.io')
                      or
                  ((k.name = '有氧運動' or k.name = '復健訓練') and u.email = 'starplatinum@hexschooltest.io');

-- 3-3 修改：更新教練的經驗年數，資料需求如下：
-- 1. 教練`肌肉棒子` 的經驗年數為3年
-- 2. 教練`Q太郎` 的經驗年數為5年
update "COACH" as c
set experience_years = t.year from(
	values
...
from "USER" as u
         join "COURSE" as c
              on c.user_id = (select id from "USER" where name = '李燕容')
where u.name in ('王小明', '好野人');

-- 5-2. 修改：`王小明`取消預約 `李燕容` 的課程，請在`COURSE_BOOKING`更新該筆預約資料：
-- 1. 取消預約時間`cancelled_at` 設為2024-11-24 17:00:00
-- 2. 狀態`status` 設定為課程已取消
update "COURSE_BOOKING" as cb
...
where
  ...
  and cb.course_id =
    ...
    join "USER" as c_u on c_u.id = c.user_id
    where c_u.name = '李燕容'
    );
...
select cp.user_id, sum(cp.price_paid) as total
...
from "COURSE_BOOKING" as cb
         join "USER" as u on u.id = cb.user_id
where u.name = '王小明'
  and cb.status != '已取消'
group by cb.user_id;
...
select cp.user_id,
       ...
       (select count(cb.id)
        ...
        where cp.user_id = cb.user_id
          and cb.status != '已取消' ) as remaining_credit
from "CREDIT_PURCHASE" as cp
    join "USER" as u
on u.id = cp.user_id
where u.name = '王小明'
...
select u.name as 教練名稱, co.experience_years as 經驗年數, s.name as 專長名稱
from "COACH" as co
         inner join "COACH_LINK_SKILL" as cls on cls.coach_id = co.id
         inner join "SKILL" as s on s.id = cls.skill_id
         inner join "USER" as u on u.id = co.user_id
where s.name = '重訓'
group by u.name, co.experience_years, s.name
order by co.experience_years DESC;

-- 6-2 查詢：查詢每種專長的教練數量，並只列出教練數量最多的專長（需使用 group by, inner join 與 order by 與 limit 語法）
-- 顯示須包含以下欄位： 專長名稱, coach_total
select s.name as 專長名稱, count(co.user_id) as coach_total
from "COACH" as co
         inner join "COACH_LINK_SKILL" as cls on cls.coach_id = co.id
         inner join "SKILL" as s on s.id = cls.skill_id
group by s.name
...
select cp.name as 組合包方案名稱, count(c_pu.id) as 銷售數量
from "CREDIT_PURCHASE" c_pu
         join "CREDIT_PACKAGE" cp on cp.id = c_pu.credit_package_id
where to_char(c_pu.purchase_at, 'YYYY-MM') = '2024-11'
group by cp.name
order by 銷售數量 desc limit 1;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
select sum(cp.price_paid) as 總營收
from "CREDIT_PURCHASE" as cp
where to_char(cp.purchase_at, 'YYYY-MM') = '2024-11';

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
select count(distinct cb.user_id) as 預約會員人數
from "COURSE_BOOKING" as cb
where to_char(cb.created_at, 'YYYY-MM') = '2024-11'
  and cb.status != '已取消';